...
func setupApp17(t *testing.T) (*service.App, *store.Store, model.Spring, model.Sensor) {
	t.Helper()
	...
	spring, err := app.CreateSpring(context.Background(), model.SpringInput{
		Code: "SPR-17", Name: "白石泉", Region: "西岭", Aquifer: "灰岩含水层",
	...
	sensor, err := app.CreateSensor(context.Background(), model.SensorInput{
		SpringID: spring.ID, SerialNo: "FLOW-17", Kind: string(model.SensorFlow), Unit: "m3/s",
...
func TestBug17_NullSensorReadingStaysNil(t *testing.T) {
	app, _, _, sensor := setupApp17(t)
	loaded, err := app.GetSensor(context.Background(), sensor.ID)
	if err != nil {
		t.Fatal(err)
	}
	if loaded.LastValue != nil {
		t.Fatalf("unread sensor has synthetic value %v", *loaded.LastValue)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, err = app.GetSensor(ctx, sensor.ID)
