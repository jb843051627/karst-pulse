...
func setupApp18(t *testing.T) (*service.App, *store.Store, model.Spring, model.Sensor) {
	t.Helper()
	...
	spring, err := app.CreateSpring(context.Background(), model.SpringInput{
		Code: "SPR-18", Name: "白石泉", Region: "西岭", Aquifer: "灰岩含水层",
	...
	sensor, err := app.CreateSensor(context.Background(), model.SensorInput{
		SpringID: spring.ID, SerialNo: "FLOW-18", Kind: string(model.SensorFlow), Unit: "m3/s",
...
func TestBug18_OptionalEventReferenceStaysNil(t *testing.T) {
	app, _, spring, _ := setupApp18(t)
	batch, err := app.CreateBatch(context.Background(), model.BatchInput{SpringID: spring.ID, BatchCode: "B-18", Collector: "field"})
	if err != nil {
		t.Fatal(err)
	}
	if batch.EventID != nil {
		t.Fatalf("nil event became %#v", batch.EventID)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, err = app.GetBatch(ctx, batch.ID)
